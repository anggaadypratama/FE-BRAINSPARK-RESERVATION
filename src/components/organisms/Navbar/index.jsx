...
const Navbar = ({className, color}) => {
	const ScreenLarge = useScreenSize({isMax: true, size: 959});
	const ScreenPhone = useScreenSize({isMax: true, size: 400});
	const classes = NavbarStyle();

	const appbarClassnames = classNames(className, classes.navbar);
	const buttonContainedClassnames = classNames(
		classes.button,
		classes.contained
	);
			...
			<Container maxWidth="lg">
				<Toolbar className={classes.toolbar}>
					...
					<div className={classes.content}>
						{!ScreenLarge &&
							menu
								?.filter(val => val.name !== "status")
								.map(({name, ...rest}) => (
									<Button
										{...rest}
										key={nanoid()}
										className={buttonDefaultClassnames}
									>
										{name}
									</Button>
								))}
						{!ScreenPhone && (
							<Button
								className={buttonContainedClassnames}
								variant="contained"
								disableElevation
							>
								status
							...
							<>
								...
								>
									<List component="nav">
										{menu
											?.filter(({name}) =>
												!ScreenPhone ? name !== "status" : name
											)
											.map(({name, link, typebutton}) => {
												if (typebutton === "link-dom") {
													return (
														<Link key={nanoid()} to={link}>
															<ListItem button className={classes.dropdownItem}>
																{name}
															</ListItem>
														</Link>
													);
